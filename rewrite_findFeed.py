...
findFeedStart = "    suspend fun findFeed(limit: Int, cursor: Instant?, viewerId: UUID? = null): List<PostDto> = dbQuery {"
findFeedEnd = "    suspend fun findById("

before = content.split(findFeedStart)[0]
after = findFeedEnd + content.split(findFeedEnd)[1]

newFindFeed = """    suspend fun findFeed(limit: Int, cursor: Instant?, viewerId: UUID? = null): List<PostDto> = dbQuery {
...
with open('backend/modules/posts/repositories/PostRepository.kt', 'w') as f:
    f.write(before + newFindFeed + after)
