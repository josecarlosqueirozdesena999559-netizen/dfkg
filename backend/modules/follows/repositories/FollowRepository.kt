package com.decisoes.modules.follows.repositories
...
class FollowRepository {
    ...
    suspend fun getFollowers(userId: UUID): List<UserProfileDto> = dbQuery {
        FollowsTable.crossJoin(UserProfilesTable)
            .selectAll()
            .where { (FollowsTable.followerId eq UserProfilesTable.userId) and (FollowsTable.followingId eq userId) }
    ...
    suspend fun getFollowing(userId: UUID): List<UserProfileDto> = dbQuery {
        FollowsTable.crossJoin(UserProfilesTable)
            .selectAll()
            .where { (FollowsTable.followingId eq UserProfilesTable.userId) and (FollowsTable.followerId eq userId) }
            .orderBy(FollowsTable.createdAt to SortOrder.DESC)
            .map { row ->
                UserProfileDto(
                    userId = row[UserProfilesTable.userId].toString(),
